...
/**
 * List Jules sessions. Returns all sessions visible to the API key.
 * @param {number} pageSize - Number of sessions to return (1-100).
 * @returns {Promise<Object>} - { sessions: [...], nextPageToken?: string }
 */
async function julesListSessions(pageSize = 100) {
  const res = await fetch(`${JULES_API_URL}/sessions?pageSize=${pageSize}`, {
    headers: julesHeaders(),
  });
  return res.json();
}
 ...
 * @param {number} pageSize - Number of activities to return (1-100).
 * @param {string} [pageToken] - Optional token for pagination.
 * @returns {Promise<Object>} - { activities: [...], nextPageToken?: string }
 */
async function julesListActivities(sessionId, pageSize = 100, pageToken = '') {
  let url = `${JULES_API_URL}/sessions/${sessionId}/activities?pageSize=${pageSize}`;
  if (pageToken) url += `&pageToken=${pageToken}`;
  const res = await fetch(url, { headers: julesHeaders() });
  return res.json();
}
